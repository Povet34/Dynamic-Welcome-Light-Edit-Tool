...
public class HashManager
{
    ...
    public static string EncryptMD5(FileStream fileStream)
    {
        var md5 = System.Security.Cryptography.MD5.Create();
    ...
    public static string EncryptSHA256(FileStream fileStream)
    {
        var sha = new System.Security.Cryptography.SHA256Managed();
        byte[] data = sha.ComputeHash(fileStream);
    ...
    public static bool IsSameHash(FileStream fileStream, string oldHash, HashType type)
    {
        string newHash = null;
        switch (type)
        {
            case HashType.MD5:
                newHash = EncryptMD5(fileStream);
                break;
            case HashType.SHA256:
                newHash = EncryptSHA256(fileStream);
                break;
            //case HashType.SHA384:
            //    newHash = EncryptSHA384(fileStream);
            //    break;
            //case HashType.SHA512:
            //    newHash = EncryptSHA512(fileStream);
            //    break;
            default:
                return false;
        }
